# modification of infillOptEASavepts that stores seen points for later xplxpl analysis
...
infillOptEASavepts = function(infill.crit, models, control, par.set, opt.path, designs, iter, ...) {
  requirePackages("emoa", why = "infillOptEASavepts")
  ...
  seen.points <- matrix(NA, nrow = control$infill.opt.restarts * (mu + lambda * control$infill.opt.ea.maxit),
                        ncol = length(x.names))
  seen.points = data.frame(seen.points, stringsAsFactors = TRUE)
  colnames(seen.points) = x.names
  ...
  for (restart.iter in seq_len(control$infill.opt.restarts)) {
    ...
    # compute position in seen.points df
    global.iter <- (control$infill.opt.ea.maxit * lambda + mu) * (restart.iter - 1) 
    # save points
    seen.points[(global.iter + 1):(global.iter + mu),] = X

      
    for (j in seq_len(control$infill.opt.ea.maxit)) {
      ...
      for (i in seq_col(parentSet)) {
        ...
        # save new individual
        seen.points[global.iter + mu + j,] = child1
  ...
  result <- list("prop.points" = best.x, "seen.points" = seen.points)
  return(result)
}
